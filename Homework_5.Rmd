...
baltimore <- homicides_2 %>% 
              ...
              group_by(year_month) %>% 
              summarize(month_homicides = n(), .groups = 'drop')
baltimore 
...
baltimore_time1 <- ggplot(baltimore, aes(x = reported_date, y= fill = temperature))+
  geom_bar() + geom_smooth(method = "auto") 

baltimore_time1

homicides_3 <- homicides_2 %>% 
...
baltimore_time + geom_smooth(se = FALSE)
